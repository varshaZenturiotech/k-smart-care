 ...
 * Retries on temporary errors (503 / 429).
 * @param {string} text 
  ...
  while (attempt < maxRetries) {
    ...
    try {
      ...
      if (response.status === 503 || response.status === 429 || response.status >= 500) {
        ...
        console.warn(`Hugging Face API returned status ${response.status} (attempt ${attempt}/${maxRetries}). Waiting ${waitMs / 1000}s. Error: ${errorData.error || response.statusText}`);
      ...
      if (attempt >= maxRetries) {
        throw new Error(`Failed to generate embedding after ${maxRetries} attempts. Last error: ${err.message}`);
      }
      console.warn(`Connection error during embedding generation (attempt ${attempt}/${maxRetries}): ${err.message}. Retrying...`);
